...
public class Train {
    public static void doTraining(String[] dati) throws IOException {
    	System.out.println("==== Train brown corpus model ====");
    	/**
    	if (args.length != 3) {
        ...
        **/
        ...
        try (CorpusReader corpusReader = Util.newCorpusReader(dati[0], new File(dati[1]))) {
        ...
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(dati[2]))) {
            oos.writeObject(model);
            System.out.println("Corpus Model DONE!");
        } catch (IOException e) {
